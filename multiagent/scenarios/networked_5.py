...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            r = 0.6
            p = 5
            landmark.state.p_pos = np.array([r*np.cos((i)*(2*np.pi)/p), r*np.sin((i)*(2*np.pi)/p)])
    ...
    def reward(self, agent, world, episode_step):
        ...
        if rew_type == 'eyes' or test == 1:
            if agent.name == 'agent 1':
                network = np.array([1, 0, 0 ,0 ,0])
            if agent.name == 'agent 2':
                network = np.array([0, 1, 0 ,0 ,0])
            if agent.name == 'agent 3':
                network = np.array([0, 0, 1 ,0 ,0])
            if agent.name == 'agent 4':
                network = np.array([0, 0, 0 ,1 ,0])
            if agent.name == 'agent 5':
                network = np.array([0, 0, 0 ,0 ,1])



        if rew_type == 'ones' and test == 0:
            if agent.name == 'agent 1':
                network = np.array([1, 1, 1 ,1 ,1])
            if agent.name == 'agent 2':
                network = np.array([1, 1, 1 ,1 ,1])
            if agent.name == 'agent 3':
                network = np.array([1, 1, 1 ,1 ,1])
        ...
        if 'network' in locals():
            ...
            for i, a in enumerate(world.agents):
                ...
                d = min(dists)
                landmark_vector.append(np.exp(-d))
                # if d < 0.3:
                #     landmark_vector.append(np.exp(-d))
                # else:
